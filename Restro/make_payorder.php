...
if (isset($_POST['make'])) {
    ...
    if (empty($_POST["order_code"]) || empty($_GET['prod_price'])) {
        ...
        // Assuming 'order_id' is auto-incremented by the database
        // No need to pass it explicitly in the INSERT query
        ...
        $postQuery = "INSERT INTO rpos_makeorders (prod_qty, order_code, prod_id, prod_name, prod_price) VALUES(?,?,?,?,?)";
        $postStmt = $mysqli->prepare($postQuery);
        //bind parameters
        $rc = $postStmt->bind_param('sssss', $prod_qty, $order_code, $prod_id, $prod_name, $prod_price);
        $postStmt->execute();   
